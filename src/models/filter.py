...
class GroupFilter(Base):
    ...
    media_type: Mapped[str | None] = mapped_column(String(32), nullable=True)  # photo, video, sticker, document, voice
    is_exact_match: Mapped[bool] = mapped_column(Boolean, default=False)
    custom_ttl: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Override TTL in seconds
