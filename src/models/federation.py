...
class Federation(Base):
    ...
    # Relationships
    admins = relationship(
        "FederationAdmin", back_populates="federation", cascade="all, delete-orphan"
    )
    groups = relationship(
        "FederationGroup", back_populates="federation", cascade="all, delete-orphan"
    )
...
class FederationAdmin(Base):
    ...
    fed_id: Mapped[str] = mapped_column(
        String(32), ForeignKey("federations.fed_id", ondelete="CASCADE"), index=True
    )
