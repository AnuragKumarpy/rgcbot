...
SESSION_STRING = os.getenv(
    "USER_SESSION_STRING",
    "1BVtsOHoBu3Qm0HDjjhqTXms7P6E5BD-WOvPAHCCqbmk8-kBVCIxc4KwqWfJRwWYQw6B_DpISsylq4t32UbWb5ADOp3xXiF2hxK8Gw8V-yGPcmDvilmZFUtLUPYCimKuNs-Ym2_iTAa9CWfDQG0DHRJTFfYgLxYwsznGXMKNEC70gMI2CphjSp-Itu2l0QiWRvzOey9Hq7tZxOytSC8tlJ_2SXK0fPR3LFIA1CS-gGwDTJ6m6Fxp-d5iPhNbbOQ4FV9veNPHo4QbJO8vEcbIF5Nr2A5RBe3_29m6I7j5gkcHkEkrgMb5_sexonlcRSgW9V4rkEfQCWXM9M95TTqXMfwVaeLZJFAo=",
)
    ...
    async def get_client(cls) -> Optional[TelegramClient]:
        async with cls._lock:
            ...
            try:
                ...
                logger.info(
                    "[MTProtoResolver] Palantir MTProto user session connected successfully."
                )
                return cls._client
    ...
    async def resolve_username(cls, username: str) -> Optional[Tuple[int, str, str]]:
        ...
        try:
            ...
            if entity and hasattr(entity, "id"):
                ...
                first_name = getattr(entity, "first_name", None) or getattr(
                    entity, "title", f"User {user_id}"
                )
                uname = getattr(entity, "username", clean_uname) or clean_uname
                logger.info(
                    f"[MTProtoResolver] Resolved @{clean_uname} -> ID {user_id} ({first_name})"
                )
